...
export class ClientDetailsComponent implements OnChanges {
   ...
   */
  ngOnChanges(changes: { clientId: SimpleChange }) {
    const { clientId } = changes;

    if (!clientId.currentValue) this.clearComponentData();

    if (clientId.currentValue && clientId.currentValue !== clientId.previousValue) {
      /** If clientId changes on selection, get the selected client details */
      this.getClientDetailsData(clientId.currentValue);
    }
  }
   ...
   */
  getClientDetailsData(clientId: string): void {
    this.apiService.getClientDetails(clientId).subscribe(
  ...
  onClientRemove(): void {
    ...
    /** Trigger the saving method from the API Service passing the clientId*/
    this.apiService.deleteClient(this.clientData.clientId).subscribe(
      () => {
